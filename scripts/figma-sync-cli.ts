...
program
    ...
    .argument('[pageId]', '생성할 페이지 ID (pages.ts의 id, 예: project.agent, 지정하지 않으면 모든 페이지 생성)')
    .action(async (pageId) => {
        try {
            ...
            if (pageId) {
                // 특정 페이지 생성
                console.log(`📄 ${pageId} 페이지 생성 중...`);
                await service.generateSinglePage(pageId);
                console.log(`✅ ${pageId} 페이지가 성공적으로 생성되었습니다.`);
            } else {
                // 모든 페이지 생성
        ...
        try {
            ...
            if (options.remote) {
                ...
                try {
                    ...
                    const pages = await client.getFilePages(platformKey);
                    console.log(`  ✅ Platform 파일의 페이지 목록 OK: ${pages.pages.length}개 페이지`);
