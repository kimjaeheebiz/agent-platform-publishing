...
/**
 * 메뉴에서 pageId 목록 추출 (메뉴 표시 여부 확인용)
 */
...
/**
 * 메뉴 구조에서 모든 라우트 정보 추출 (재귀)
 * mainmenu.ts의 path가 우선 적용됨
 */
 ...
 * 
 * 우선순위:
 * 1. 메뉴에 있는 페이지: mainmenu.ts의 path 우선 (없으면 pages.ts의 path)
 * 2. 메뉴에 없는 페이지: pages.ts의 path 사용
 */
export const ALL_ROUTES: RouteInfo[] = (() => {
    const routes: RouteInfo[] = [];
    const menuPageIds = new Set<string>();

    // 1. 메뉴에 있는 페이지: mainmenu.ts의 path 우선
    MAIN_MENUS.forEach((menu) => {
        const menuRoutes = extractRoutesFromMenu(menu);
        menuRoutes.forEach((route) => {
            routes.push(route);
            if (route.pageId) {
                menuPageIds.add(route.pageId);
            }
        });
    });
...
export const getBreadcrumbPath = (url: string): Array<{ title: string; path?: string }> => {
    ...
    // 메뉴 구조에서 URL과 일치하는 항목 찾기
    const foundMenu = findMenuByUrl(url, MAIN_MENUS);

    // 2. 메뉴에서 못 찾은 경우, pages.ts에서 페이지 정보 찾기
    if (!foundMenu) {
