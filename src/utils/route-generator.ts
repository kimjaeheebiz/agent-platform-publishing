...
const generateComponentLoader = (id: string): PageModuleLoader => {
    ...
    if (!matched) {
        ...
        if (import.meta.env.DEV) {
            ...
            console.error(
                `Available modules:`,
                Object.keys(PAGE_MODULES).filter((k) => k.includes(lastSegment)),
            );
        }

        // Fallback 컴포넌트
        const FallbackComponent: React.FC = () => React.createElement('div', null, `Page not found: ${id}`);
...
export const generateAllRoutes = (): RouteObject[] => {
    ...
    allRoutes.forEach((route) => {
        ...
        if (layout === 'auth') {
            ...
            // ErrorLayout 라우트 (statusCode를 URL에서 파싱)
            // /404 → 404, /500 → 500, 그 외 → 404
            let statusCode = 404;
            if (url.includes('/500') || url === '/500') {
                statusCode = 500;
            } else if (url.includes('/404') || url === '/404') {
                statusCode = 404;
            }
