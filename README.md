...
# 특정 페이지 생성 (pages.ts의 id 사용)
npm run figma:page -- <pageId>
# 예: npm run figma:page -- project.agent
...
3) `npm run figma:page -- project.agent` 실행으로 콘텐츠 생성
4) `src/config/mainmenu.ts`에서 `pageId: 'project.agent'` 참조하여 메뉴 연동
5) 레이아웃/네비게이션과 자동 통합
